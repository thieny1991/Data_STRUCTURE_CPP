...
res1=$(date +%s.%N)
./sort "input=inputA.txt;command=command31.txt;output=output31.txt" 1>1.stdcout 2>1.stderr
res2=$(date +%s.%N)
...
# For test case 2
res1=$(date +%s.%N)
./sort "input=inputD.txt;command=command32.txt;output=output32.txt" 1>1.stdcout 2>1.stderr
res2=$(date +%s.%N)
diff=`echo "$res2 - $res1"|bc|awk '{printf "%f",$0}'`
desired=0.01015
error=`echo "$desired * 0.3"|bc|awk '{printf "%f",$0}'`
...
# For test case 3
res1=$(date +%s.%N)
./sort "input=inputD.txt;command=command33.txt;output=output33.txt" 1>1.stdcout 2>1.stderr
res2=$(date +%s.%N)
diff=`echo "$res2 - $res1"|bc|awk '{printf "%f",$0}'`
desired=0.0777
error=`echo "$desired * 0.3"|bc|awk '{printf "%f",$0}'`
